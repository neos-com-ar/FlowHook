...
import {
  ...
  extractTargetMeta,
  logEvent as logEntityLinkEvent,
} from './entity-linking';
import {
  compareValues,
  parseSpecPairs,
 ...
 * @param {Object} [params.incomingHeaders] Headers del webhook entrante (lowercase)
...
export async function executeWebhook({
  ...
  // ---------------------------------------------------------------------------
  // PASO 0: Inicialización de entity linking (si está habilitado en el flow)
  // ---------------------------------------------------------------------------
  const correlationId = newCorrelationId();
  const now = buildNowHelper();
  const entityLinkingActive = isEntityLinkingEnabled(flow);
  let entityLinkState = {
    enabled: false,
    mapping: null,
    mappingKey: null,
        ...
        const prevResponse = await axios({
          ...
          params:
            ...
            Object.keys(prevQueryParams).length > 0
              ? prevQueryParams
              : undefined,
          headers: resolveHeaders(prevEndpoint.headers, prevHeaderContext),
          timeout: 30000,
        });
  ...
  if (
    ...
    if (!conditionsPass) {
      ...
      // Siempre devolvemos registro para historial; el HTTP status refleja el fallo.
      const webhookResult = {
        success: false,
        status: 400,
  ...
  if (flow.headers && typeof flow.headers === 'object') {
    for (const [headerKey, headerValue] of Object.entries(flow.headers)) {
      headers[headerKey] = resolveHeaderValue(headerValue, combinedData);
    }
  }

  if (flow.webhookSecret && typeof flow.webhookSecret === 'string') {
  ...
  // Registrar mapping pendiente (si entity linking está habilitado en el flow).
  if (entityLinkingActive && userId) {
    try {
      entityLinkState = await beginMapping({
        userId,
        flowId,
        projectId,
        flow,
      ...
      logEntityLinkEvent('begin_mapping_error', {
  ...
  let postResponseActionsResults = [];

  const runPostResponseActions = async ({ destinationSucceeded, context }) => {
    if (
    ...
    for (const action of flow.postResponseActions) {
      const shouldRun = destinationSucceeded || action.onlyOnSuccess === false;
      if (!shouldRun) {
        results.push({
          ...
          skipped: true,
          reason:
            'Action skipped because destination endpoint failed and onlyOnSuccess is enabled',
        });
        continue;
      }

      try {
        const actionResult = await executePostResponseAction(action, context);
        ...
        const errorResult = {
          ...
          status: actionError.status || actionError.response?.status || 500,
          data: actionError.responseData || actionError.response?.data || null,
          url: actionError.request?.url,
          method: actionError.request?.method,
          request: actionError.request || null,
        };
  ...
  try {
    ...
    // Finalizar el entity_mapping con los datos del destino antes de ejecutar
    // las postResponseActions, para poder inyectar `entityLink` en su contexto.
    let updatedMapping = entityLinkState.mapping;
    if (entityLinkState.enabled && entityLinkState.mappingKey) {
      const ctxWithResponse = {
        data: incomingData,
        headers: incomingHeaders,
        prev: prevData,
        now,
      ...
      const targetMeta = extractTargetMeta(flow, ctxWithResponse);
      try {
        updatedMapping = await finalizeMapping({
          userId,
          flowId,
          projectId,
          mappingKey: entityLinkState.mappingKey,
          status: 'linked',
          targetMeta,
          correlationId: entityLinkState.correlationId,
        });
        logEntityLinkEvent('linking_latency', {
          correlation_id: entityLinkState.correlationId,
          user_id: userId,
          flow_id: flowId,
          mapping_key: entityLinkState.mappingKey,
          linking_latency_ms: Date.now() - linkingStartedAt,
        });
      } catch (linkError) {
        logEntityLinkEvent('finalize_mapping_error', {
          correlation_id: entityLinkState.correlationId,
          user_id: userId,
          flow_id: flowId,
          mapping_key: entityLinkState.mappingKey,
    ...
    const entityLinkContext = buildEntityLinkContext(updatedMapping);
    const actionContext = {
      response: response.data || {},
      data: incomingData,
      headers: incomingHeaders,
      prev: prevData,
      now,
      ...(entityLinkContext ? { entityLink: entityLinkContext } : {}),
    };
    postResponseActionsResults = await runPostResponseActions({
      destinationSucceeded: true,
      context: actionContext,
    });
    webhookResult.postResponseActions = postResponseActionsResults;
    ...
    const webhookRecord = buildBaseWebhookRecord({
      ...
      headers,
      result: webhookResult,
      postResponseActions: postResponseActionsResults,
      manual,
      mode,
      originalWebhook,
    });
    ...
    const entityLinkContext = buildEntityLinkContext(entityLinkState.mapping);
    const actionContext = {
      response: {},
      data: incomingData,
      headers: incomingHeaders,
    ...
    // Finalizar el entity_mapping con failed/retrying según clasificación del error.
    if (entityLinkState.enabled && entityLinkState.mappingKey) {
      const classified = classifyError(error);
      const finalStatus =
        classified.kind === 'transient' ? 'retrying' : 'failed';
      try {
        await finalizeMapping({
          userId,
          flowId,
          projectId,
          mappingKey: entityLinkState.mappingKey,
...
function buildBaseWebhookRecord({
  ...
  const base = {
    incomingData,
    incomingHeaders:
      incomingHeaders && Object.keys(incomingHeaders).length > 0
        ? incomingHeaders
        : undefined,
    prevData:
...
function parseSourceKey(sourceKey) {
  ...
  if (cleanKey.includes('::case{') && cleanKey.endsWith('}')) {
    const caseMatch = cleanKey.match(/::case\{([^}]+)\}$/);
    if (caseMatch) {
      const caseString = caseMatch[1];
      cleanKey = cleanKey.replace(/::case\{[^}]+\}$/, '');
      result.caseSpec = parseSpecPairs(caseString);
    }
  }
  ...
  suffixes.forEach((suffix) => {
    ...
    const suffixTransform = parseSuffixTransform(trimmed);
    if (suffixTransform) {
      if (suffixTransform.type === 'if') {
        result.ifTransform = suffixTransform;
      } else if (isNumericTransformType(suffixTransform.type)) {
        result.numericTransforms.push(suffixTransform);
      }
      return;
    }
    ...
    const isnullStringMatch = trimmed.match(/^isnull\(\s*["']([^"']*)["']\s*\)$/);
    if (isnullStringMatch) {
      result.transforms.push({ type: 'isnull', defaultValue: isnullStringMatch[1] });
      return;
    }

    const isnullNumberMatch = trimmed.match(/^isnull\(\s*(-?\d+(?:\.\d+)?)\s*\)$/);
    if (isnullNumberMatch) {
      const num = Number(isnullNumberMatch[1]);
      result.transforms.push({ type: 'isnull', defaultValue: num });
      return;
    }
...
function applyStringTransforms(value, transforms) {
  ...
  for (const transform of transforms) {
    if (current === undefined || current === null) {
      if (transform.type === 'isnull') {
        current = transform.defaultValue;
      } else {
        break;
      }
      continue;
    ...
    switch (transform.type) {
      ...
      case 'substr':
        ...
        break;
      case 'isnull':
        // Solo se aplica cuando current es null/undefined (ya manejado arriba)
        break;
      default:
...
function applyValueTransforms(value, parsed, context) {
  const {
    transforms,
    numericTransforms,
    ifTransform,
  ...
  if (transforms && transforms.length > 0) {
    current = applyStringTransforms(current, transforms);
  }

  if (numericTransforms && numericTransforms.length > 0) {
    for (const numericTransform of numericTransforms) {
      current = applyNumericTransform(
        current,
        numericTransform,
        context,
        getNestedValue,
  ...
  if (ifTransform) {
    current = resolveIf({
      value: current,
      operator: ifTransform.operator,
      compareTo: ifTransform.compareTo,
      thenVal: ifTransform.thenVal,
      elseVal: ifTransform.elseVal,
      context,
      getNestedValue,
    });
  }
  ...
  if (caseSpec) {
    current = resolveCase(current, caseSpec);
  } else if (mapSpec && mapSpec[current] !== undefined) {
    current = mapSpec[current];
  }

  if (toNumber || toInt) {
    const numValue = Number(current);
    if (!Number.isNaN(numValue)) {
      current = toInt ? Math.trunc(numValue) : numValue;
    }
  }
...
function replaceTemplateFunctions(template, webhookData) {
  const fnPattern = /\{\{\s*(if|case)\(/g;
  const replacements = [];
  let match;

  fnPattern.lastIndex = 0;
  while ((match = fnPattern.exec(template)) !== null) {
    ...
    if (template.substring(closeParenIndex + 1, closeParenIndex + 3) !== '}}') {
      continue;
    }

    const argsString = template.substring(openParenIndex + 1, closeParenIndex);
    const fullStart = match.index;
    const fullEnd = closeParenIndex + 3;

    let resolvedValue;
    if (fnName === 'if') {
      resolvedValue = evaluateTemplateIf(argsString, webhookData, getNestedValue);
    } else {
      resolvedValue = evaluateTemplateCase(argsString, webhookData, getNestedValue);
    }

    replacements.push({
      start: fullStart,
      end: fullEnd,
      replacement: formatTemplateValue(resolvedValue),
    });
  }
    ...
    const prevResponse = await axios({
      ...
      params:
        ...
        Object.keys(prevQueryParams).length > 0
          ? prevQueryParams
          : undefined,
      headers: resolveHeaders(prevEndpoint.headers, webhookData),
      timeout: 30000,
    });
...
// Resuelve el contenido de {{ expresión }} igual que una clave de mapeo:
// ruta base, sufijos :: (trim, isnull, etc.), ::map{}, ::number, ::int y arrays con [].
function resolveTemplatePlaceholder(webhookData, expression) {
  const parsed = parseSourceKey(String(expression));
  const { baseKey, isArrayMapping } = parsed;

  const value = getNestedValue(webhookData, baseKey);
  ...
  return applyValueTransforms(value, parsed, webhookData);
}

function processTemplate(template, webhookData, options = {}) {
  let result = replaceTemplateFunctions(template, webhookData);

  const resolvePlaceholder = (expression) =>
    resolveTemplatePlaceholder(webhookData, expression);

  result = resolveConcatenatedStringTemplates(result, resolvePlaceholder);

  result = resolveInlineCalcTemplates(result, resolvePlaceholder);

  result = resolveAdjacentUnquotedPlaceholders(result, resolvePlaceholder);

  result = result.replace(/\{\{([^}]+)\}\}/g, (match, path) => {
    const value = resolveTemplatePlaceholder(webhookData, path);
    return formatTemplateValue(value);
  });

  if (options.resolveArithmetic) {
    result = resolveArithmeticInJsonString(result);
  }

  return result;
}

function resolveHeaderValue(value, context) {
  if (typeof value !== 'string') {
    return value;
  }
  ...
  return value.replace(/\{\{([^}]+)\}\}/g, (match, expression) => {
    const resolvedValue = resolveTemplatePlaceholder(context, expression);
...
function resolveHeaders(headersConfig, context) {
  ...
  for (const [headerKey, headerValue] of Object.entries(headersConfig)) {
    resolvedHeaders[headerKey] = resolveHeaderValue(headerValue, context);
  }
...
async function processTemplateAsync(template, webhookData, prevEndpoints) {
  ...
  if (matches.length === 0) {
    return processTemplate(template, webhookData, { resolveArithmetic: true });
  }
...
function parseLiteralBodyMapValue(literalValue) {
  if (
  ...
  if (!isNaN(literalValue) && literalValue.trim() !== '') {
...
function resolveBodyMapValue(sourceKey, context) {
  if (!sourceKey || sourceKey.trim() === '') {
    return undefined;
  }
  ...
  if (trimmed.startsWith('literal:')) {
    let literalValue = trimmed.substring(8).trim();
    literalValue = processTemplate(literalValue, context);
    try {
      return parseLiteralBodyMapValue(literalValue);
    } catch (parseError) {
      console.warn('Error parsing literal bodyMap value:', parseError);
      return literalValue;
    }
  }

  if (trimmed.includes('{{')) {
    const resolved = processTemplate(trimmed, context);
    ...
    if (!isNaN(resolved) && resolved.trim() !== '') {
  ...
  if (trimmed === 'true' || trimmed === 'false' || trimmed === 'null') {
    return parseLiteralBodyMapValue(trimmed);
  }
  ...
  return resolveTemplatePlaceholder(context, trimmed);
}

function buildPostResponseActionRequest(action, context) {
  let actionUrl = action.url;
  actionUrl = processUrlTemplate(actionUrl, context);
  if (actionUrl.includes('{{') || actionUrl.includes('}}')) {
    throw new Error(
      `Action URL has unresolved template variables: ${actionUrl}`,
    );
  }
  ...
  try {
    ...
    throw new Error(`Invalid action URL after template processing: ${actionUrl}`);
  }
  ...
  if (action.bodyMap && typeof action.bodyMap === 'object') {
    for (const [actionKey, sourceKey] of Object.entries(action.bodyMap)) {
      if (sourceKey && sourceKey.trim() !== '') {
        const value = resolveBodyMapValue(sourceKey, context);
        if (value !== undefined) {
  ...
  const headers = resolveHeaders(action.headers, context);
...
async function executePostResponseAction(action, context) {
  const request = buildPostResponseActionRequest(action, context);
  ...
  try {
    ...
    const actionError = new Error(error.message);
    actionError.status = error.response?.status || 500;
    actionError.responseData = error.response?.data || null;
    actionError.request = request;
    throw actionError;
  }
}
...
async function processArrayLiteral(
  ...
  if (!detection.shouldIterate) {
    ...
    return processTemplate(literalTemplate, combinedData, { resolveArithmetic: true });
  }
...
// - Cálculos con prefijo calc:
// - Plantillas {{ }} sobre data/prev con if/case y aritmética inline
...
async function applyMapping(flowMap, combinedData, mappedData, flow, prevEndpoints) {
  for (const [destKey, sourceKey] of Object.entries(flowMap)) {
    ...
    if (typeof sourceKey === 'string' && sourceKey.startsWith('calc:')) {
      let calcExpr = sourceKey.substring(5).trim();
      calcExpr = processTemplate(calcExpr, combinedData);
      const calcResult = evaluateArithmeticExpression(calcExpr);
      mappedData[destKey] = calcResult;
      continue;
    }
    ...
    if (typeof sourceKey === 'string' && sourceKey.startsWith('literal:')) {
      ...
      if (value !== undefined) {
        if (finalIsArrayMapping && Array.isArray(value)) {
          ...
          mappedData[destKey] = applyValueTransforms(value, parsed, combinedData);
        }
      } else if (
        parsed.transforms?.length ||
        parsed.numericTransforms?.length ||
        parsed.ifTransform ||
        parsed.caseSpec
      ) {
        value = applyValueTransforms(value, parsed, combinedData);
        if (value !== undefined) {
          mappedData[destKey] = value;
        }
      }
