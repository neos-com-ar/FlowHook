...
export async function handleWebhookIngress(request, { userId, workspaceId, projectId, flowId, legacy = false }) {
  ...
  try {
    ...
    // Guardar en historial los fallos de endpoint previo requerido.
    if (error.__flowType === 'prevEndpointRequired') {
      try {
        ...
        await saveWebhook(userId || flow.ownerId, flowId, failureRecord, projectId);
      } catch (saveError) {
        console.error(
          `[FlowHook] Failed to persist history for prevEndpointRequired on flow ${flowId}:`,
          saveError,
        );
      }
