...
async function migrateBucketToSummaries(userId, flowId, projectId, bucket) {
  ...
  return bucket.map((entry) =>
    isFullWebhookRecord(entry) ? toWebhookSummary(entry) : entry,
  );
}
...
async function rebuildFlowWebhookIndexFromBucket(userId, flowId, projectId, flowName) {
  ...
  const index = enrichWebhooksList(webhooks, flowMap).map(toWebhookSummary);
  ...
  try {
    ...
    if (useKV && kv) {
      try {
        ...
        const bucket = appendWebhookToBucket(preparedBucket, summary);
...
export async function getWebhooks(userId, flowId = null, limit = 100, offset = 0, filters = {}) {
  try {
    const { status, startDate, endDate, projectId, summary = true } = filters || {};
    ...
    const paginateFromIndex = (webhooks) => {
    ...
    if (summary) {
      if (flowId) {
        ...
        return paginateFromIndex(webhooks);
      }

      const allWebhooks = await loadAllWebhooksFromIndex(userId);
      return paginateFromIndex(allWebhooks);
    }
