...
const ROLE_HIERARCHY = { owner: 3, admin: 2, editor: 2, viewer: 1 };
...
export async function createWorkspace(userId, workspaceData) {
  ...
  const slug = workspaceData.slug || slugifyWorkspaceName(name);
...
export async function deleteWorkspace(workspaceId, userId) {
  ...
  if (projectIds.length > 0) {
    throw new Error('No se puede eliminar un workspace con proyectos. Mueve o elimina los proyectos primero.');
  }
