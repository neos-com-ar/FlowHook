...
async function loadWebhooksForFlowFromKv(userId, flowId, projectId, flowName) {
  if (projectId) {
    const scopedKey = getScopedWebhookKvKey(userId, flowId, projectId);
    const scopedWebhooks = (await kv.get(scopedKey)) || [];
    const legacyWebhooks = (await kv.get(getLegacyWebhookKvKey(userId, flowId))) || [];
    const filteredLegacy = legacyWebhooks.filter((webhook) =>
      matchesProjectFilter(webhook, projectId, flowName),
    );
    return mergeAndSortWebhooks(scopedWebhooks, filteredLegacy);
  }

  const projectFlows = await getAllFlowsForUser(userId);
  const matchingFlows = projectFlows.filter((flow) => flow.id === flowId);
  const arrays = matchingFlows.map(async (flow) =>
    (await kv.get(getScopedWebhookKvKey(userId, flowId, flow.projectId))) || [],
  );
  arrays.push(kv.get(getLegacyWebhookKvKey(userId, flowId)) || []);
  const resolved = await Promise.all(arrays);
  return mergeAndSortWebhooks(...resolved);
}

function loadWebhooksForFlowFromLocal(data, userId, flowId, projectId, flowName) {
  const userWebhooks = data[userId]?.webhooks || {};

  if (projectId) {
    const scopedKey = getScopedWebhookLocalKey(flowId, projectId);
    const scopedWebhooks = userWebhooks[scopedKey] || [];
    const legacyWebhooks = userWebhooks[flowId] || [];
    const filteredLegacy = legacyWebhooks.filter((webhook) =>
      matchesProjectFilter(webhook, projectId, flowName),
    );
    return mergeAndSortWebhooks(scopedWebhooks, filteredLegacy);
  ...
  const arrays = [];
  for (const [key, webhooks] of Object.entries(userWebhooks)) {
...
async function loadAllWebhooksFromKv(userId) {
  ...
  for (const flow of projectFlows) {
    const scopedKey = getScopedWebhookKvKey(userId, flow.id, flow.projectId);
    const webhooks = (await kv.get(scopedKey)) || [];
    for (const webhook of webhooks) {
  ...
  const legacyFlowIds = new Set([
    ...projectFlows.map((flow) => flow.id),
    ...oldFlows.map((flow) => flow.id),
  ]);

  for (const flowId of legacyFlowIds) {
    const legacyWebhooks = (await kv.get(getLegacyWebhookKvKey(userId, flowId))) || [];
    for (const webhook of legacyWebhooks) {
      if (!webhook.projectId && !seenIds.has(webhook.id)) {
        seenIds.add(webhook.id);
        allWebhooks.push(webhook);
      }
    }
...
function loadAllWebhooksFromLocal(data, userId) {
  ...
  for (const webhooks of Object.values(userWebhooks)) {
    for (const webhook of webhooks) {
      if (!seenIds.has(webhook.id)) {
        seenIds.add(webhook.id);
        allWebhooks.push(webhook);
...
export async function saveWebhook(userId, flowId, webhookData, projectId = null) {
  try {
    ...
    if (useKV && kv) {
      try {
        const key = getScopedWebhookKvKey(userId, flowId, projectId);
        ...
        const localKey = getScopedWebhookLocalKey(flowId, projectId);
        const bucket = data[userId].webhooks[localKey] || [];
        data[userId].webhooks[localKey] = appendWebhookToBucket(bucket, webhook);
        await saveLocalData(data);
        return true;
      }
    }
...
export async function updateWebhook(userId, flowId, webhookId, updater, projectId = null) {
  try {
    ...
    if (useKV && kv) {
      try {
        const scopedKey = getScopedWebhookKvKey(userId, flowId, projectId);
        let webhooks = (await kv.get(scopedKey)) || [];
        let result = await tryUpdateBucket(webhooks);
        if (result.updated) {
          await kv.set(scopedKey, result.webhooks);
          return true;
        }

        const legacyKey = getLegacyWebhookKvKey(userId, flowId);
        webhooks = (await kv.get(legacyKey)) || [];
        result = await tryUpdateBucket(webhooks);
        if (result.updated) {
          await kv.set(legacyKey, result.webhooks);
          return true;
        }
        ...
        const scopedLocalKey = getScopedWebhookLocalKey(flowId, projectId);
        let webhooks = data[userId].webhooks[scopedLocalKey] || [];
        let result = await tryUpdateBucket(webhooks);
        if (result.updated) {
          data[userId].webhooks[scopedLocalKey] = result.webhooks;
          await saveLocalData(data);
          return true;
        }

        webhooks = data[userId].webhooks[flowId] || [];
        result = await tryUpdateBucket(webhooks);
        if (result.updated) {
          data[userId].webhooks[flowId] = result.webhooks;
          await saveLocalData(data);
          return true;
        }
 ...
 */
function buildEntityMappingKey(userId, flowId, mappingKey) {
 ...
 */
export async function getEntityMapping(userId, flowId, mappingKey) {
  if (!userId || !flowId || !mappingKey) {
    return null;
  }

  const storageKey = buildEntityMappingKey(userId, flowId, mappingKey);
  ...
  try {
    if (useKV && kv) {
      try {
        ...
        const data = await getLocalData();
        return data.entityMappings?.[storageKey] || null;
      }
    } else {
      const data = await getLocalData();
      return data.entityMappings?.[storageKey] || null;
 ...
 */
export async function upsertEntityMapping(userId, flowId, mappingKey, updater) {
  ...
  try {
    if (useKV && kv) {
      try {
        const existing = (await kv.get(storageKey)) || null;
        ...
        const existing = data.entityMappings[storageKey] || null;
        const next = updater(existing);
        if (!next) {
          return existing;
        }
...
export async function listEntityMappings(userId, flowId, options = {}) {
  ...
  const { mappingKey, limit = 50 } = options;

  if (mappingKey) {
    const single = await getEntityMapping(userId, flowId, mappingKey);
    return single ? [single] : [];
  }

  const prefix = `entity_mapping:${userId}:${flowId}:`;
  const normalizedLimit = Math.max(1, Math.min(Number(limit) || 50, 200));

  try {
    if (useKV && kv) {
      try {
        // Upstash/Vercel KV suele soportar KEYS(pattern). Si no, devolvemos vacío.
        const keys = (await kv.keys(`${prefix}*`)) || [];
        if (!Array.isArray(keys) || keys.length === 0) return [];

        const values = await Promise.all(
          keys.slice(0, normalizedLimit).map((k) => kv.get(k)),
        );
        return values
          .filter(Boolean)
          .sort(
