...
const WEBHOOK_GLOBAL_INDEX_MAX = 500;
const WEBHOOK_FLOW_INDEX_MAX = 1000;
// Upstash/Vercel KV suele rechazar valores cercanos o superiores a 1MB.
const WEBHOOK_BUCKET_MAX_BYTES = 900_000;
const WEBHOOK_SUMMARY_SEARCH_TEXT_MAX = 2_000;
const WEBHOOK_RECORD_MIGRATE_BATCH = 25;

function getProjectWebhookIndexKey(projectId) {
  return `webhook_index:project_all:${projectId}`;
}

function getUserWebhookIndexKey(userId) {
  return `webhook_index:user_all:${userId}`;
}

function getFlowWebhookIndexKey(userId, flowId, projectId) {
  if (projectId) {
    return `webhook_index:flow:${projectId}:${flowId}`;
  }
  return `webhook_index:flow:user:${userId}:${flowId}`;
}

function getFlowWebhookIndexLocalKey(userId, flowId, projectId) {
  if (projectId) {
    return `${projectId}:${flowId}`;
  }
  return `user:${userId}:${flowId}`;
}

function getFlowWebhookMetaKey(userId, flowId, projectId) {
  if (projectId) {
    return `webhook_meta:flow:${projectId}:${flowId}`;
  }
  return `webhook_meta:flow:user:${userId}:${flowId}`;
}

function getWebhookRecordKvKey(userId, flowId, projectId, webhookId) {
  if (projectId) {
    return `webhook_record:${projectId}:${flowId}:${webhookId}`;
  }
  return `webhook_record:user:${userId}:${flowId}:${webhookId}`;
}

function getWebhookRecordLocalKey(userId, flowId, projectId, webhookId) {
  if (projectId) {
    return `${projectId}:${flowId}:${webhookId}`;
  }
  return `user:${userId}:${flowId}:${webhookId}`;
}

function isFullWebhookRecord(entry) {
  return Boolean(
    entry &&
      (entry.incomingData !== undefined ||
        entry.mappedData !== undefined ||
        entry.incomingHeaders !== undefined ||
        entry.headers !== undefined ||
        entry.prevData !== undefined ||
        entry.postResponseActions !== undefined ||
        entry.result?.responseData !== undefined ||
        entry.result?.postResponseActions !== undefined ||
        (typeof entry.searchText === 'string' &&
          entry.searchText.length > WEBHOOK_SUMMARY_SEARCH_TEXT_MAX)),
  );
}

function bucketApproximateBytes(bucket) {
  try {
    return JSON.stringify(bucket).length;
  } catch {
    return Number.MAX_SAFE_INTEGER;
  }
}

function ensureWebhookRecordsBucket(data) {
  if (!data.webhookRecords) {
    data.webhookRecords = {};
  }
}

async function persistWebhookRecord(userId, flowId, projectId, webhook) {
  if (useKV && kv) {
    await kv.set(getWebhookRecordKvKey(userId, flowId, projectId, webhook.id), webhook);
    return;
  }

  const data = await getLocalData();
  ensureWebhookRecordsBucket(data);
  data.webhookRecords[getWebhookRecordLocalKey(userId, flowId, projectId, webhook.id)] =
    webhook;
  await saveLocalData(data);
}

async function loadWebhookRecord(userId, flowId, projectId, webhookId) {
  if (useKV && kv) {
    return (await kv.get(getWebhookRecordKvKey(userId, flowId, projectId, webhookId))) || null;
  }

  const data = await getLocalData();
  ensureWebhookRecordsBucket(data);
  return (
    data.webhookRecords[getWebhookRecordLocalKey(userId, flowId, projectId, webhookId)] ||
    null
  );
}

async function migrateBucketToSummaries(userId, flowId, projectId, bucket) {
  if (!Array.isArray(bucket) || bucket.length === 0) {
    return [];
  }

  const fullEntries = bucket.filter(isFullWebhookRecord);
  for (let i = 0; i < fullEntries.length; i += WEBHOOK_RECORD_MIGRATE_BATCH) {
    const batch = fullEntries.slice(i, i + WEBHOOK_RECORD_MIGRATE_BATCH);
    await Promise.all(
      batch.map((entry) => persistWebhookRecord(userId, flowId, projectId, entry)),
    );
  }

  return bucket.map((entry) => {
    if (isFullWebhookRecord(entry)) {
      return toWebhookSummary(entry);
    }
    return toWebhookSummary(entry);
  });
}

function trimWebhookBucketToLimit(bucket, maxBytes = WEBHOOK_BUCKET_MAX_BYTES) {
  let prepared = Array.isArray(bucket) ? [...bucket] : [];
  while (
    prepared.length > 0 &&
    bucketApproximateBytes(prepared) > maxBytes
  ) {
    prepared.pop();
  }
  return prepared;
...
function parseWebhookDateFilter(dateValue, endOfDay = false) {
  if (!dateValue) {
    return null;
  }
...
async function prepareWebhookBucketForSave(userId, flowId, projectId, bucket) {
  let prepared = bucket || [];
  const needsMigration =
    prepared.some(isFullWebhookRecord) ||
    bucketApproximateBytes(prepared) > WEBHOOK_BUCKET_MAX_BYTES;

  if (needsMigration) {
    prepared = await migrateBucketToSummaries(userId, flowId, projectId, prepared);
  }

  return trimWebhookBucketToLimit(prepared);
}

function appendToWebhookIndex(index, entry, max) {
  const filtered = index.filter((item) => item.id !== entry.id);
  filtered.unshift(entry);
  return filtered.slice(0, max);
}

function ensureWebhookIndexesBucket(data) {
  if (!data.webhookIndexes) {
    data.webhookIndexes = {
      projectAll: {},
      flow: {},
      userAll: {},
...
async function buildWebhookIndexEntry(userId, webhook, projectId) {
  const entry = toWebhookSummary(webhook);
  ...
  if (projectId) {
    ...
    const flow = flows.find((item) => item.id === webhook.flowId);
    if (project) {
      entry.projectId = projectId;
      entry.projectName = project.name;
      entry.projectIcon = project.icon;
      entry.projectColor = project.color;
    }
    if (flow) {
      entry.flowName = flow.name;
    }
    return entry;
  }
...
async function updateFlowWebhookMeta(userId, flowId, projectId, webhooks) {
  const meta = {
    count: webhooks.length,
    latestTimestamp: webhooks[0]?.timestamp || null,
    latestId: webhooks[0]?.id || null,
  };

  if (useKV && kv) {
    await kv.set(getFlowWebhookMetaKey(userId, flowId, projectId), meta);
    return;
  }

  const data = await getLocalData();
  ensureWebhookIndexesBucket(data);
  if (!data.webhookIndexes.meta) {
    data.webhookIndexes.meta = {};
  }
  data.webhookIndexes.meta[getFlowWebhookMetaKey(userId, flowId, projectId)] = meta;
  await saveLocalData(data);
}

async function readFlowWebhookMeta(userId, flowId, projectId) {
  if (useKV && kv) {
    return (await kv.get(getFlowWebhookMetaKey(userId, flowId, projectId))) || null;
  }

  const data = await getLocalData();
  ensureWebhookIndexesBucket(data);
  return data.webhookIndexes.meta?.[getFlowWebhookMetaKey(userId, flowId, projectId)] || null;
}

function isFlowWebhookIndexStale(index, meta) {
  ...
  if (meta.count > index.length) {
    return true;
  }
  if (meta.latestId && index[0]?.id && meta.latestId !== index[0].id) {
    return true;
  }
  if (meta.latestTimestamp && index[0]?.timestamp) {
    return new Date(meta.latestTimestamp) > new Date(index[0].timestamp);
  }
  return false;
}

async function writeFlowWebhookIndex(userId, flowId, projectId, flowLocalKey, index) {
  const trimmed = index.slice(0, WEBHOOK_FLOW_INDEX_MAX);

  if (useKV && kv) {
    await kv.set(getFlowWebhookIndexKey(userId, flowId, projectId), trimmed);
    return trimmed;
  }

  const data = await getLocalData();
  ensureWebhookIndexesBucket(data);
  data.webhookIndexes.flow[flowLocalKey] = trimmed;
  await saveLocalData(data);
  return trimmed;
}

async function mergeFlowIndexIntoProjectIndex(projectId, flowIndex) {
  if (!projectId || flowIndex.length === 0) {
    return;
  }

  if (useKV && kv) {
    const projectIndex = (await kv.get(getProjectWebhookIndexKey(projectId))) || [];
    const merged = mergeAndSortWebhooks(projectIndex, flowIndex).slice(
      0,
      WEBHOOK_GLOBAL_INDEX_MAX,
    );
    await kv.set(getProjectWebhookIndexKey(projectId), merged);
    return;
  }

  const data = await getLocalData();
  ensureWebhookIndexesBucket(data);
  const projectIndex = data.webhookIndexes.projectAll[projectId] || [];
  data.webhookIndexes.projectAll[projectId] = mergeAndSortWebhooks(
    projectIndex,
    flowIndex,
  ).slice(0, WEBHOOK_GLOBAL_INDEX_MAX);
  await saveLocalData(data);
}

async function rebuildFlowWebhookIndexFromBucket(userId, flowId, projectId, flowName) {
  let webhooks = [];
  if (useKV && kv) {
    webhooks = await loadWebhooksForFlowFromKv(userId, flowId, projectId, flowName);
  } else {
    const data = await getLocalData();
    webhooks = loadWebhooksForFlowFromLocal(data, userId, flowId, projectId, flowName);
  }

  if (webhooks.length === 0) {
    await updateFlowWebhookMeta(userId, flowId, projectId, []);
    return [];
  }

  const flowMap = await buildFlowMapForContext(userId, flowId, projectId);
  const index = enrichWebhooksList(webhooks, flowMap).map((webhook) =>
    toWebhookSummary(webhook),
  );
  const flowLocalKey = getFlowWebhookIndexLocalKey(userId, flowId, projectId);
  const trimmed = await writeFlowWebhookIndex(
    userId,
    flowId,
    projectId,
    flowLocalKey,
  ...
  await mergeFlowIndexIntoProjectIndex(projectId, trimmed);
  await updateFlowWebhookMeta(userId, flowId, projectId, webhooks);
  return trimmed;
}

async function upsertWebhookIndex(userId, webhook, projectId = null) {
  const entry = await buildWebhookIndexEntry(userId, webhook, projectId);
  const flowIndexKey = getFlowWebhookIndexKey(userId, webhook.flowId, projectId);
  const flowLocalKey = getFlowWebhookIndexLocalKey(userId, webhook.flowId, projectId);
  const globalIndexKey = projectId
    ? getProjectWebhookIndexKey(projectId)
    : getUserWebhookIndexKey(userId);
  const globalIndexMax = projectId ? WEBHOOK_GLOBAL_INDEX_MAX : WEBHOOK_GLOBAL_INDEX_MAX;

  const applyIndexUpdates = async () => {
    if (useKV && kv) {
      const flowIndex = (await kv.get(flowIndexKey)) || [];
      await kv.set(
        flowIndexKey,
        appendToWebhookIndex(flowIndex, entry, WEBHOOK_FLOW_INDEX_MAX),
      );

      const globalIndex = (await kv.get(globalIndexKey)) || [];
      await kv.set(
        globalIndexKey,
        appendToWebhookIndex(globalIndex, entry, globalIndexMax),
      );
      return;
    }
    ...
    const flowIndex = data.webhookIndexes.flow[flowLocalKey] || [];
    data.webhookIndexes.flow[flowLocalKey] = appendToWebhookIndex(
      flowIndex,
      entry,
      WEBHOOK_FLOW_INDEX_MAX,
    );

    if (projectId) {
      const projectIndex = data.webhookIndexes.projectAll[projectId] || [];
      data.webhookIndexes.projectAll[projectId] = appendToWebhookIndex(
        projectIndex,
        entry,
        WEBHOOK_GLOBAL_INDEX_MAX,
      );
    } else {
      const userIndex = data.webhookIndexes.userAll[userId] || [];
      data.webhookIndexes.userAll[userId] = appendToWebhookIndex(
        userIndex,
        entry,
        WEBHOOK_GLOBAL_INDEX_MAX,
      );
  ...
  const verifyFlowIndexContainsEntry = async () => {
    if (useKV && kv) {
      const flowIndex = (await kv.get(flowIndexKey)) || [];
      return flowIndex.some((item) => item.id === entry.id);
    }
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    await applyIndexUpdates();
    if (await verifyFlowIndexContainsEntry()) {
      return;
    }
  }

  console.warn(
    `[FlowHook] Index update verification failed for webhook ${entry.id} in flow ${webhook.flowId}`,
  );
}

async function rebuildAllWebhookIndexes(userId, projects) {
  const allWebhooks = await loadAllWebhooksFromKv(userId);
  if (allWebhooks.length === 0) {
    return [];
  }

  const flowMap = await buildFlowMapForContext(userId, null, null);
  const summaries = enrichWebhooksList(allWebhooks, flowMap).map(toWebhookSummary);
  const byProject = new Map();
  const legacy = [];
  const byFlow = new Map();

  for (const entry of summaries) {
    const flowKey = entry.projectId
      ? getFlowWebhookIndexLocalKey(userId, entry.flowId, entry.projectId)
      : getFlowWebhookIndexLocalKey(userId, entry.flowId, null);

    if (!byFlow.has(flowKey)) {
      byFlow.set(flowKey, []);
    }
    byFlow.get(flowKey).push(entry);
  ...
  if (useKV && kv) {
    ...
    for (const project of projects) {
      const projectEntries = sortByTimestamp(byProject.get(project.id) || []).slice(
        0,
        WEBHOOK_GLOBAL_INDEX_MAX,
      );
      writeTasks.push(kv.set(getProjectWebhookIndexKey(project.id), projectEntries));
    }

    writeTasks.push(
      kv.set(
        getUserWebhookIndexKey(userId),
        sortByTimestamp(legacy).slice(0, WEBHOOK_GLOBAL_INDEX_MAX),
      ),
    );

    for (const [flowKey, entries] of byFlow) {
      const sorted = sortByTimestamp(entries).slice(0, WEBHOOK_FLOW_INDEX_MAX);
      if (flowKey.startsWith('user:')) {
        const flowId = flowKey.split(':').slice(2).join(':');
        writeTasks.push(kv.set(getFlowWebhookIndexKey(userId, flowId, null), sorted));
      } else {
        const separatorIndex = flowKey.indexOf(':');
        const projectId = flowKey.slice(0, separatorIndex);
        const flowId = flowKey.slice(separatorIndex + 1);
        writeTasks.push(kv.set(getFlowWebhookIndexKey(userId, flowId, projectId), sorted));
      }
    }
  ...
  for (const project of projects) {
    data.webhookIndexes.projectAll[project.id] = sortByTimestamp(
      byProject.get(project.id) || [],
    ).slice(0, WEBHOOK_GLOBAL_INDEX_MAX);
  }

  data.webhookIndexes.userAll[userId] = sortByTimestamp(legacy).slice(
    0,
    WEBHOOK_GLOBAL_INDEX_MAX,
  );

  for (const [flowKey, entries] of byFlow) {
    data.webhookIndexes.flow[flowKey] = sortByTimestamp(entries).slice(
      0,
      WEBHOOK_FLOW_INDEX_MAX,
    );
  }
...
async function loadAllWebhooksFromIndex(userId) {
  const projects = await getUserProjects(userId);

  const loadMergedProjectIndexes = async () => {
    if (useKV && kv) {
      const keys = [
        ...projects.map((project) => getProjectWebhookIndexKey(project.id)),
        getUserWebhookIndexKey(userId),
      ];
      const indexes = await Promise.all(keys.map((key) => kv.get(key)));
      return mergeAndSortWebhooks(...indexes.map((index) => index || []));
    }

    const data = await getLocalData();
    ensureWebhookIndexesBucket(data);
    const localIndexes = [
      ...projects.map((project) => data.webhookIndexes.projectAll[project.id] || []),
      data.webhookIndexes.userAll[userId] || [],
    ];
    return mergeAndSortWebhooks(...localIndexes);
  };

  const rebuildFromFlowIndexes = async () => {
    const flowIndexLoads = [];

    for (const project of projects) {
      const flows = await getProjectFlows(project.id);
      for (const flow of flows) {
        flowIndexLoads.push(
          loadFlowWebhookIndex(userId, flow.id, project.id, flow.name),
        );
      }
    }

    const legacyFlows = await getUserFlows(userId);
    for (const flow of legacyFlows) {
      flowIndexLoads.push(loadFlowWebhookIndex(userId, flow.id, null, flow.name));
    }

    const flowIndexes = await Promise.all(flowIndexLoads);
    return mergeAndSortWebhooks(...flowIndexes);
  };

  const isProjectIndexStaleAgainstMetas = async (merged) => {
    const latestIndexTs = merged[0]?.timestamp
      ? new Date(merged[0].timestamp).getTime()
      : 0;
    let latestMetaTs = 0;
    ...
    for (const project of projects) {
      ...
      const metas = await Promise.all(
        flows.map((flow) => readFlowWebhookMeta(userId, flow.id, project.id)),
      );
      for (const meta of metas) {
        if (meta?.latestTimestamp) {
          latestMetaTs = Math.max(
            latestMetaTs,
    ...
    return latestMetaTs > latestIndexTs;
  };

  const merged = await loadMergedProjectIndexes();

  if (merged.length === 0) {
    return rebuildAllWebhookIndexes(userId, projects);
  }

  if (await isProjectIndexStaleAgainstMetas(merged)) {
    console.warn(
      `[FlowHook] Project webhook index stale for user ${userId}; rebuilding from flow indexes`,
    );
    const rebuilt = await rebuildFromFlowIndexes();
    if (rebuilt.length > 0) {
      // Actualiza índices de proyecto/usuario a partir de los índices de flujo ya sincronizados.
      if (useKV && kv) {
        const byProject = new Map();
        const legacy = [];
        ...
        const writeTasks = projects.map((project) =>
          kv.set(
            getProjectWebhookIndexKey(project.id),
            (byProject.get(project.id) || [])
...
async function loadFlowWebhookIndex(userId, flowId, projectId, flowName) {
  const flowIndexKey = getFlowWebhookIndexKey(userId, flowId, projectId);
  const flowLocalKey = getFlowWebhookIndexLocalKey(userId, flowId, projectId);

  let index = [];
  if (useKV && kv) {
    index = (await kv.get(flowIndexKey)) || [];
  } else {
    const data = await getLocalData();
    ensureWebhookIndexesBucket(data);
    index = data.webhookIndexes.flow[flowLocalKey] || [];
  }

  const meta = await readFlowWebhookMeta(userId, flowId, projectId);
  const shouldRebuild =
    !meta ||
    isFlowWebhookIndexStale(index, meta) ||
    (index.length === 0 && (meta?.count || 0) > 0);

  if (shouldRebuild) {
    index = await rebuildFlowWebhookIndexFromBucket(userId, flowId, projectId, flowName);
  }

  if (projectId) {
    return index.filter((webhook) => matchesProjectFilter(webhook, projectId, flowName));
  }
  return index;
}
...
async function resolveFlowNameForFilter(userId, flowId, projectId) {
  ...
  const flow = projectFlows.find((item) => item.id === flowId);
  return flow?.name || null;
}

function toWebhookSummary(webhook) {
  ...
  const result = webhook.result
    ? {
        success: webhook.result.success,
        status: webhook.result.status,
        message: webhook.result.message,
        error: webhook.result.error,
        responseTime: webhook.result.responseTime,
      }
    : undefined;

  const searchSource =
    typeof webhook.searchText === 'string' && webhook.searchText.length > 0
      ? webhook.searchText
      : buildWebhookSearchText(webhook);
  ...
  return {
    ...
    flowName: webhook.flowName,
    projectName: webhook.projectName,
    projectIcon: webhook.projectIcon,
    projectColor: webhook.projectColor,
    destino: webhook.destino,
    method: webhook.method,
    manualRetry: webhook.manualRetry,
    retryCount: webhook.retryCount,
    lastRetryAt: webhook.lastRetryAt,
    result,
    searchText:
      typeof searchSource === 'string'
        ? searchSource.slice(0, WEBHOOK_SUMMARY_SEARCH_TEXT_MAX)
        : '',
  };
}

function buildWebhookSearchText(webhook) {
  if (!webhook || typeof webhook !== 'object') {
    return '';
  }
  ...
  try {
    const parts = [
      ...
      webhook.result?.responseData,
      webhook.incomingData,
      webhook.mappedData,
      webhook.incomingHeaders,
      webhook.headers,
      webhook.prevData,
      webhook.postResponseActions,
      webhook.result?.postResponseActions,
    ];
    ...
    return text.slice(0, WEBHOOK_SUMMARY_SEARCH_TEXT_MAX);
  } catch {
    return '';
  }
}

function withSearchText(webhook, { forSummary = false } = {}) {
  if (!webhook || typeof webhook !== 'object') {
    return webhook;
  }
  ...
  if (webhook.searchText) {
    return webhook;
  }
  return {
    ...webhook,
    searchText: buildWebhookSearchText(webhook),
  };
}

async function webhookMatchesSearch(webhook, searchTerm, userId, projectId) {
  const term = String(searchTerm || '').trim().toLowerCase();
  if (!term) {
    return true;
  }

  const flowProjectId = webhook.projectId || projectId || null;
  const haystack =
    webhook.searchText ||
    buildWebhookSearchText(webhook);
  ...
  const record = await loadWebhookRecord(
    userId,
    webhook.flowId,
    flowProjectId,
    webhook.id,
  ...
  return buildWebhookSearchText(record).includes(term);
}

async function applySearchFilter(webhooks, search, userId, projectId) {
  ...
  for (let i = 0; i < webhooks.length; i += batchSize) {
    ...
    const checks = await Promise.all(
      batch.map(async (webhook) => {
        const isMatch = await webhookMatchesSearch(webhook, term, userId, projectId);
        return isMatch ? webhook : null;
      }),
    );
...
async function buildFlowMapForContext(userId, flowId = null, projectId = null) {
  if (projectId && flowId) {
    const [project, flows] = await Promise.all([
      getProject(projectId),
      getProjectFlows(projectId),
    ...
    if (project && flow) {
      ...
      return buildFlowInfoMap(projectFlows, []);
    }
  }
...
async function loadWebhookBucketFromKv(userId, flowId, projectId) {
  ...
  if (projectId) {
    keys.push(getProjectWebhookKvKey(projectId, flowId));
    keys.push(getScopedWebhookKvKey(userId, flowId, projectId));
  }

  keys.push(getLegacyWebhookKvKey(userId, flowId));
  const buckets = await Promise.all(keys.map((key) => kv.get(key)));
  return mergeAndSortWebhooks(...buckets.map((bucket) => bucket || []));
}
...
async function loadAllWebhooksFromKv(userId) {
  const [projects, oldFlows] = await Promise.all([
    getUserProjects(userId),
    getUserFlows(userId),
  ]);

  const projectFlowPairs = (
    await Promise.all(
      projects.map(async (project) => {
        const flows = await getProjectFlows(project.id);
        return flows.map((flow) => ({ project, flow }));
      }),
    )
  ).flat();

  const loadTasks = [
    ...projectFlowPairs.map(({ project, flow }) =>
      loadWebhookBucketFromKv(userId, flow.id, project.id).then((webhooks) =>
        webhooks.map((webhook) => ({
          ...webhook,
          projectId: webhook.projectId || project.id,
        })),
      ),
    ),
    ...oldFlows.map((flow) => loadWebhookBucketFromKv(userId, flow.id, null)),
  ];

  const arrays = await Promise.all(loadTasks);
  return mergeAndSortWebhooks(...arrays);
}
  ...
  try {
    ...
    if (useKV && kv) {
      try {
        ...
        await persistWebhookRecord(userId, flowId, projectId, webhook);

        const writeBucketAndIndexes = async (existingBucket) => {
          const preparedBucket = await prepareWebhookBucketForSave(
            userId,
            flowId,
            projectId,
            existingBucket,
          );
          const summary = toWebhookSummary(webhook);
          const bucket = trimWebhookBucketToLimit(
            appendWebhookToBucket(preparedBucket, summary),
          );
          await kv.set(key, bucket);
          await updateFlowWebhookMeta(userId, flowId, projectId, bucket);
          await upsertWebhookIndex(userId, webhook, projectId);
          return true;
        };

        try {
          const existingBucket = (await kv.get(key)) || [];
          return await writeBucketAndIndexes(existingBucket);
        } catch (firstError) {
          console.error(
            'Error al guardar bucket de webhook en KV, reintentando con compactación agresiva:',
            firstError,
          );
          // Reintento: forzar migración a resúmenes livianos y recompactar.
          const existingBucket = (await kv.get(key)) || [];
          const migrated = await migrateBucketToSummaries(
            userId,
            flowId,
            projectId,
            existingBucket,
          );
          const aggressive = trimWebhookBucketToLimit(
            migrated.map((entry) => toWebhookSummary(entry)),
            Math.floor(WEBHOOK_BUCKET_MAX_BYTES * 0.75),
          );
          await kv.set(key, aggressive);
          return await writeBucketAndIndexes(aggressive);
        }
      } catch (error) {
        console.error('Error al guardar webhook en KV:', error);
        return false;
      }
    }
    ...
    if (projectId) {
      ...
      const existingBucket = data.projectWebhooks[localKey] || [];
      const preparedBucket = await prepareWebhookBucketForSave(
        userId,
        flowId,
        projectId,
      ...
      data.projectWebhooks[localKey] = trimWebhookBucketToLimit(
        appendWebhookToBucket(preparedBucket, toWebhookSummary(webhook)),
      );
    } else {
      ensureUserWebhooksBucket(data, userId);
      const existingBucket = data[userId].webhooks[flowId] || [];
      const preparedBucket = await prepareWebhookBucketForSave(
        userId,
        flowId,
        projectId,
      ...
      data[userId].webhooks[flowId] = trimWebhookBucketToLimit(
        appendWebhookToBucket(preparedBucket, toWebhookSummary(webhook)),
      );
    }
    await saveLocalData(data);
    const savedBucket = projectId
      ? data.projectWebhooks[getProjectWebhookLocalKey(projectId, flowId)] || []
      : data[userId].webhooks[flowId] || [];
    await updateFlowWebhookMeta(userId, flowId, projectId, savedBucket);
    await upsertWebhookIndex(userId, webhook, projectId);
    return true;
  } catch (error) {
      ...
      try {
        ...
        for (const key of keysToTry) {
          ...
          if (result.updated) {
            ...
            const updatedWebhook = result.webhooks.find((item) => item.id === webhookId);
            if (updatedWebhook) {
              await upsertWebhookIndex(userId, updatedWebhook, projectId);
            }
            return true;
          }
        }
...
/**
 * Diagnóstico de almacenamiento de webhooks para un flujo.
 * Compara bucket (datos completos), índice (listado) y metadatos.
 */
export async function getWebhookStorageDiagnostics(userId, flowId, projectId = null) {
  const bucketKey = projectId
    ? getProjectWebhookKvKey(projectId, flowId)
    : getLegacyWebhookKvKey(userId, flowId);
  const flowIndexKey = getFlowWebhookIndexKey(userId, flowId, projectId);
  const projectIndexKey = projectId ? getProjectWebhookIndexKey(projectId) : null;
  const metaKey = getFlowWebhookMetaKey(userId, flowId, projectId);

  const summarizeBucket = (bucket) => ({
    count: bucket.length,
    latestTimestamp: bucket[0]?.timestamp || null,
    latestId: bucket[0]?.id || null,
    approximateBytes: JSON.stringify(bucket).length,
  });

  if (useKV && kv) {
    const [bucket, flowIndex, projectIndex, meta] = await Promise.all([
      kv.get(bucketKey),
      kv.get(flowIndexKey),
      projectIndexKey ? kv.get(projectIndexKey) : Promise.resolve(null),
      kv.get(metaKey),
    ]);
    const bucketList = bucket || [];
    const flowIndexList = flowIndex || [];
    const projectIndexList = projectIndex || [];
    const approximateBytes = bucketApproximateBytes(bucketList);
    const fullRecordsInBucket = bucketList.filter(isFullWebhookRecord).length;
    const flowMatchesInProjectIndex = projectId
      ? projectIndexList.filter((entry) => entry.flowId === flowId).length
      : null;
    ...
    return {
      ...
      keys: {
        bucket: bucketKey,
        flowIndex: flowIndexKey,
        projectIndex: projectIndexKey,
        meta: metaKey,
      },
      bucket: {
        ...summarizeBucket(bucketList),
        fullRecordsInBucket,
        overSizeLimit: approximateBytes > WEBHOOK_BUCKET_MAX_BYTES,
        saveLikelyBlocked:
          approximateBytes > WEBHOOK_BUCKET_MAX_BYTES || fullRecordsInBucket > 0,
      },
      flowIndex: {
        count: flowIndexList.length,
        latestTimestamp: flowIndexList[0]?.timestamp || null,
        latestId: flowIndexList[0]?.id || null,
      },
      projectIndex: projectIndexKey
        ? {
            totalCount: projectIndexList.length,
            flowMatches: flowMatchesInProjectIndex,
          }
        : null,
      meta: meta || null,
      inSync:
        (meta?.count || bucketList.length) === flowIndexList.length &&
        (!meta?.latestId || meta.latestId === flowIndexList[0]?.id),
    };
  }

  const data = await getLocalData();
  const bucketList = projectId
    ? data.projectWebhooks?.[getProjectWebhookLocalKey(projectId, flowId)] || []
    : data[userId]?.webhooks?.[flowId] || [];
  ensureWebhookIndexesBucket(data);
  const flowLocalKey = getFlowWebhookIndexLocalKey(userId, flowId, projectId);
  const flowIndexList = data.webhookIndexes.flow[flowLocalKey] || [];
  const meta = data.webhookIndexes.meta?.[metaKey] || null;

  return {
    storage: 'local',
    keys: {
      bucket: projectId
        ? getProjectWebhookLocalKey(projectId, flowId)
        : `user:${userId}:${flowId}`,
      flowIndex: flowLocalKey,
      meta: metaKey,
    },
    bucket: summarizeBucket(bucketList),
    flowIndex: {
      count: flowIndexList.length,
      latestTimestamp: flowIndexList[0]?.timestamp || null,
      latestId: flowIndexList[0]?.id || null,
...
/**
 * Compacta el bucket de un flujo: migra registros completos a claves individuales
 * y deja en el bucket solo resúmenes livianos.
 */
export async function compactWebhookStorageForFlow(userId, flowId, projectId = null) {
  const bucketKey = projectId
    ? getProjectWebhookKvKey(projectId, flowId)
    : getLegacyWebhookKvKey(userId, flowId);
  ...
  const beforeBytes = bucketApproximateBytes(bucket);
  const fullRecordsMigrated = bucket.filter(isFullWebhookRecord).length;
  const prepared = trimWebhookBucketToLimit(
    (await prepareWebhookBucketForSave(userId, flowId, projectId, bucket)).map((entry) =>
      toWebhookSummary(entry),
    ),
  );

  if (useKV && kv) {
    await kv.set(bucketKey, prepared);
  } else {
    const data = await getLocalData();
    if (projectId) {
      ensureProjectWebhooksBucket(data);
      data.projectWebhooks[getProjectWebhookLocalKey(projectId, flowId)] = prepared;
    } else {
      ensureUserWebhooksBucket(data, userId);
      data[userId].webhooks[flowId] = prepared;
    }
    await saveLocalData(data);
  }

  await updateFlowWebhookMeta(userId, flowId, projectId, prepared);
  const flowName = await resolveFlowNameForFilter(userId, flowId, projectId);
  await rebuildFlowWebhookIndexFromBucket(userId, flowId, projectId, flowName);

  return {
    count: prepared.length,
    beforeBytes,
    afterBytes: bucketApproximateBytes(prepared),
    fullRecordsMigrated,
  };
}

async function deleteWebhookRecordKeys(userId, flowId, projectId, webhookIds) {
  ...
  if (useKV && kv) {
    await Promise.all(
      webhookIds.map((webhookId) =>
        kv.del(getWebhookRecordKvKey(userId, flowId, projectId, webhookId)),
      ),
    );
    return;
  }
  ...
  for (const webhookId of webhookIds) {
    delete data.webhookRecords[
      getWebhookRecordLocalKey(userId, flowId, projectId, webhookId)
    ];
  }
  await saveLocalData(data);
}

async function deleteWebhooksFromDateForFlow(userId, flowId, projectId, cutoffDate) {
  ...
  if (useKV && kv) {
    ...
    for (const key of keys) {
      ...
      if (key === (projectId ? getProjectWebhookKvKey(projectId, flowId) : getLegacyWebhookKvKey(userId, flowId))) {
        keptBucket = kept;
      } else if (keptBucket.length === 0) {
        keptBucket = kept;
      }
    }
  } else {
    ...
    if (projectId) {
      ...
      touchBuckets.push({
        store: 'project',
        key: getProjectWebhookLocalKey(projectId, flowId),
      });
      ensureUserWebhooksBucket(data, userId);
      touchBuckets.push({
        store: 'user',
        key: getScopedWebhookLocalKey(flowId, projectId),
      });
    }
    ensureUserWebhooksBucket(data, userId);
    touchBuckets.push({ store: 'user', key: flowId });

    for (const { store, key } of touchBuckets) {
      const bucket =
        store === 'project'
          ? data.projectWebhooks[key] || []
          : data[userId].webhooks[key] || [];
      ...
      if (store === 'project') {
        data.projectWebhooks[key] = kept;
      } else {
        data[userId].webhooks[key] = kept;
      }

      if (
        (projectId && store === 'project' && key === getProjectWebhookLocalKey(projectId, flowId)) ||
        (!projectId && store === 'user' && key === flowId)
      ) {
        keptBucket = kept;
      }
    }
  ...
  await deleteWebhookRecordKeys(userId, flowId, projectId, Array.from(removedIds));
  await updateFlowWebhookMeta(userId, flowId, projectId, keptBucket);
  const flowName = await resolveFlowNameForFilter(userId, flowId, projectId);
  await rebuildFlowWebhookIndexFromBucket(userId, flowId, projectId, flowName);
  ...
  return {
    ...
    remainingCount: keptBucket.length,
  };
}

/**
 * Borra logs de webhooks anteriores a una fecha (exclusive).
 * Si se pasa flowId, solo afecta ese flujo; si no, todos los flujos del usuario.
 *
 * @param {string} userId
 * @param {string} beforeDate - YYYY-MM-DD o ISO; se borran logs con timestamp < inicio de ese día (UTC)
 * @param {{ flowId?: string|null, projectId?: string|null }} [options]
 * @returns {Promise<{ deletedCount: number, beforeDate: string, flows: Object[] }>}
 */
export async function deleteWebhooksBeforeDate(userId, beforeDate, options = {}) {
  const { flowId = null, projectId = null } = options || {};
  const cutoffDate = parseWebhookDateFilter(beforeDate, false);
  ...
  const targets = [];
  if (flowId) {
    targets.push({ flowId, projectId: projectId || null });
  } else {
    const [projects, legacyFlows] = await Promise.all([
      getUserProjects(userId),
      getUserFlows(userId),
    ]);
    ...
    for (const project of projects) {
      const flows = await getProjectFlows(project.id);
      for (const flow of flows) {
        targets.push({ flowId: flow.id, projectId: project.id });
      }
    }
    for (const flow of legacyFlows) {
      targets.push({ flowId: flow.id, projectId: null });
    }
  }
  ...
  for (const target of targets) {
    const result = await deleteWebhooksFromDateForFlow(
      userId,
      target.flowId,
      target.projectId,
      cutoffDate,
  ...
  if (!flowId) {
    const projects = await getUserProjects(userId);
    await rebuildAllWebhookIndexes(userId, projects);
  }
...
/** @deprecated Usar deleteWebhooksBeforeDate */
export async function deleteWebhooksFromDate(userId, fromDate, options = {}) {
  return deleteWebhooksBeforeDate(userId, fromDate, options);
}
...
export async function getWebhooks(userId, flowId = null, limit = 100, offset = 0, filters = {}) {
  try {
    const { status, startDate, endDate, projectId, summary = true, search } = filters || {};
    ...
    const applyFilters = (webhooks) => {
      ...
      if (startDate) {
        const start = parseWebhookDateFilter(startDate, false);
        filtered = filtered.filter((webhook) => new Date(webhook.timestamp) >= start);
      }

      if (endDate) {
        const end = parseWebhookDateFilter(endDate, true);
    ...
    const paginateFromIndex = async (webhooks) => {
      let filtered = applyFilters(webhooks);
      if (search && String(search).trim()) {
        filtered = await applySearchFilter(filtered, search, userId, projectId);
      }
      return {
    ...
    const paginateAndEnrich = async (webhooks) => {
      const flowMap = await buildFlowMapForContext(userId, flowId, projectId);
      const filtered = applyFilters(webhooks);
      let page = enrichWebhooksList(
        filtered.slice(offset, offset + limit),
        flowMap,
      );
      if (summary) {
        page = page.map(toWebhookSummary);
      }
      return {
        webhooks: page,
        total: filtered.length,
    ...
    // Listado rápido: usa índices livianos (sin deserializar payloads completos).
    if (summary) {
      if (flowId) {
        const flowName = projectId
          ? (await resolveFlowNameForFilter(userId, flowId, projectId))
          : null;
        const webhooks = await loadFlowWebhookIndex(
          userId,
          flowId,
          projectId,
          flowName,
        );
        return await paginateFromIndex(webhooks);
      }

      const allWebhooks = await loadAllWebhooksFromIndex(userId);
      return await paginateFromIndex(allWebhooks);
    }
    ...
    if (useKV && kv) {
      try {
        ...
        const allWebhooks = await loadAllWebhooksFromKv(userId);
        return paginateAndEnrich(allWebhooks);
      } catch (error) {
...
export async function getWebhookById(userId, webhookId, flowId, projectId = null) {
  try {
    const storedRecord = await loadWebhookRecord(userId, flowId, projectId, webhookId);
    if (storedRecord) {
      const flowMap = await buildFlowMapForContext(userId, flowId, projectId);
      return enrichSingleWebhook(storedRecord, flowMap);
    }
    ...
    if (useKV && kv) {
      try {
        webhooks = await loadWebhooksForFlowFromKv(userId, flowId, projectId, null);
      } catch (error) {
        console.error('Error al obtener webhook de KV, usando fallback local:', error);
        const data = await getLocalData();
        webhooks = loadWebhooksForFlowFromLocal(data, userId, flowId, projectId, null);
      }
    } else {
      const data = await getLocalData();
      webhooks = loadWebhooksForFlowFromLocal(data, userId, flowId, projectId, null);
    ...
    const flowMap = await buildFlowMapForContext(userId, flowId, projectId);
    return enrichSingleWebhook(webhook, flowMap);
  } catch (error) {
