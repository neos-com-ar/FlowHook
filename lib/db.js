...
const WEBHOOK_BUCKET_MAX_BYTES = 4_000_000;
...
async function migrateBucketToSummaries(userId, flowId, projectId, bucket) {
  ...
  return bucket.map((entry) => {
    if (isFullWebhookRecord(entry)) {
      const summary = toWebhookSummary(entry);
      summary.searchText = buildWebhookSearchText(entry);
      return summary;
    }
    return withSearchText(entry);
  });
}
...
async function rebuildFlowWebhookIndexFromBucket(userId, flowId, projectId, flowName) {
  ...
  const index = enrichWebhooksList(webhooks, flowMap).map((webhook) =>
    withSearchText(toWebhookSummary(webhook)),
  );
...
async function loadAllWebhooksFromIndex(userId) {
  ...
  if (useKV && kv) {
    ...
    const merged = mergeAndSortWebhooks(...indexes.map((index) => index || []));
  ...
  const merged = mergeAndSortWebhooks(...localIndexes);
...
function withSearchText(webhook) {
  if (!webhook || typeof webhook !== 'object') {
    return webhook;
  }
  ...
  try {
    ...
    if (useKV && kv) {
      try {
        ...
        const bucket = appendWebhookToBucket(preparedBucket, withSearchText(summary));
    ...
    if (projectId) {
      ...
      data.projectWebhooks[localKey] = appendWebhookToBucket(
        preparedBucket,
        withSearchText(toWebhookSummary(webhook)),
      );
    } else {
      ...
      data[userId].webhooks[flowId] = appendWebhookToBucket(
        preparedBucket,
        withSearchText(toWebhookSummary(webhook)),
      );
    }
...
export async function compactWebhookStorageForFlow(userId, flowId, projectId = null) {
  ...
  const prepared = await prepareWebhookBucketForSave(userId, flowId, projectId, bucket);
