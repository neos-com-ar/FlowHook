...
/** Fragmento de texto al concatenar placeholders dentro de un string JSON. */
export function formatConcatenatedTemplatePart(value) {
...
/**
 * Resuelve {{...}} dentro de strings JSON entre comillas dobles.
 * Permite concatenar varios placeholders y texto fijo en un mismo campo.
 */
export function resolveConcatenatedStringTemplates(template, resolvePlaceholder) {
  ...
  while (i < template.length) {
    ...
    while (j < template.length) {
      if (template[j] === '\\' && j + 1 < template.length) {
        rawContent += template[j] + template[j + 1];
        j += 2;
        continue;
      }
      if (template[j] === '"') {
    ...
    if (rawContent.includes('{{')) {
      const resolved = rawContent.replace(/\{\{([^}]+)\}\}/g, (_match, expr) =>
        formatConcatenatedTemplatePart(resolvePlaceholder(expr.trim())),
      );
      result += JSON.stringify(resolved);
    } else {
      result += template.slice(i, j + 1);
    }
...
/**
 * Concatena secuencias de placeholders sin comillas: {{a}}{{b}}{{c}}.
 */
export function resolveAdjacentUnquotedPlaceholders(template, resolvePlaceholder) {
  if (!template || typeof template !== 'string' || !template.includes('{{')) {
    return template;
  }

  return template.replace(/(\{\{[^}]+\}\})+/g, (match) => {
    const placeholders = [...match.matchAll(/\{\{([^}]+)\}\}/g)];
    ...
    for (const [, expr] of placeholders) {
      concatenated += formatConcatenatedTemplatePart(resolvePlaceholder(expr.trim()));
    }
