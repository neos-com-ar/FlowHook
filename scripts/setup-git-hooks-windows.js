...
try {
  ...
  // Crear el contenido del hook (compatible con Windows usando node directamente)
  const newHookContent = `#!/bin/sh
# Incrementar build number antes de cada commit
node "${incrementBuildScript.replace(/\\/g, '/')}"
...
`;
  ...
  console.log('📝 Nota: Si usas Git Bash o WSL, el hook funcionará automáticamente.');
  console.log('   Si usas otro cliente Git, puede que necesites configurar el hook manualmente.');
} catch (error) {
